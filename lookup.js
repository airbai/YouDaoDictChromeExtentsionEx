...
var timer, prevC, prevO, prevWord, c ,selectWord;
...
document.getElementById("wordbook") ? document.getElementById("wordbook").onclick = function (e) {
    chrome.extension.sendRequest({ 'action': 'addword', 'word': document.getElementById("yddKeyTitle").innerHTML }, onAddedWord);
}:null;
...
function onAddedWord(isOK) {
    if (isOK) {
        document.getElementById("wordbook").style.color = "#8AC007";
    }
    else {
        document.getElementById("wordbook").style.color = "#FB5755";

        window.open("http://account.youdao.com/login?back_url=http://dict.youdao.com&service=dict");
    }
    setTimeout(function () {
        document.getElementById("wordbook").style.color = "#50799b";
    }, 1000);
}
